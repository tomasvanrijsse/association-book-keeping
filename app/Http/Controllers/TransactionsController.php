...
class TransactionsController extends CI_Controller {
    ...
    public function hideSuggestion(){
        ...
        $cs = new counterSuggestion($this->input->post('suggestie_id'));
        if(is_numeric($cs->transactie_id)){
            $t = new transaction($cs->transactie_id);
            $t->delete();
        }
    ...
    public function saveSuggestion(){
        ...
        $debet = new transaction($this->input->post('debet_id'));
        $credit = new transaction($this->input->post('credit_id'));
